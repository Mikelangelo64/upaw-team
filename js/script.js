$(document).ready(function () {
  ...
  //team banner swipe
  const teamBanner = document.querySelector('.banner-team');

  let idAnimationBack = null;
  let xDown = null;
  let totalSwipeX = {
    current: 50,
    target: 50,
  ...
  const lerp = (current, target, ease, approximationLeft = 0.001) => {
  ...
  const stopAnimation = (idAnimation) => {
    cancelAnimationFrame(idAnimation);
  };

  const backAnimation = (swipe) => {
    if (!teamBanner) {
      return;
    }
    if (document.body.clientWidth > 830) {
      teamBanner.style.backgroundPosition = '';
    }

    totalSwipeX.target = swipe;
    totalSwipeX.current = lerp(
      totalSwipeX.current,
      totalSwipeX.target,
      0.15,
      0.001
    ...
    teamBanner.style.backgroundPosition = `${totalSwipeX.current}% 50%`;

    if (totalSwipeX.current === totalSwipeX.target) {
      stopAnimation(idAnimationBack);
    } else {
      backAnimation(totalSwipeX.target);
    }
  };
  ...
  const handleTouchStart = (evt) => {
    const firstTouch = getTouhes(evt)[0];
  ...
  const handleTouchMove = (evt) => {
    ...
    let swipe = totalSwipeX.current;

    if (xDiff > 0) {
      //console.log('right');
      swipe = Math.min(swipe + 1, 100);
    } else {
      //console.log('left');
      swipe = Math.max(swipe - 1, 0);
    }

    xDown = xUp;

    idAnimationBack = window.requestAnimationFrame(() => {
      backAnimation(swipe);
    });
  };
  ...
  if (teamBanner && document.body.clientWidth <= 830) {
    teamBanner.addEventListener('touchstart', handleTouchStart);
    teamBanner.addEventListener('touchmove', handleTouchMove);
    teamBanner.addEventListener('touchend', handleTouchEnd);
  }
  ...
  $(window).resize(function () {
    if (
      ...
      $('.menu__form__container').hasClass('_search-active')
    ) {
      $('.menu-btns .menu__help').css('opacity', '1');
      $('.menu-btns .menu__burger').css('opacity', '1');
      $('.menu-btns .menu__lang').css('opacity', '1');
  ...
  $('.menu__link').click(function (e) {
    ...
    if (
      ...
      $(this).hasClass('footer-main-category__title__dop')
    )
      return;
  ...
  function onAnchorClick(event) {
    ...
    if (goto && $(goto)) {
      $([document.documentElement, document.body]).animate(
        {
          scrollTop:
            $(goto).offset().top /* - Math.round($('.header').height() )*/,
        },
        500
      );
    }
    ...
    if (
      $('.menu-big').hasClass('_menu-big-opened') ||
      $('.pop-up').hasClass('_pop-up-opened')
    ) {
      $('.menu-big').removeClass('_menu-big-opened');
  ...
  if (document.body.clientWidth <= 830) {
    ...
    $('.footer-main-category__title.footer-main-category__title__dop').click(
      function (e) {
        e.preventDefault();
        $(this).toggleClass('_active-dropdown');
        ...
        $('.footer-main-category__title.footer-main-category__title__dop')
          .not($(this))
          .removeClass('_active-dropdown');
        $('.footer-main-category__list')
          .not($(this).next($('.footer-main-category__list')))
          .slideUp(300);
      }
    );
  }
  ...
  function activateToggleSection(section) {
    $(`${section} .faq-toggle-content__item`)
      .not($(`${section} .faq-toggle-content__item._active-page`))
      .fadeOut(200);
    ...
    $(`${section} .faq-toggle-head__item`).click(function (event) {
      ...
      if (goto && $(goto)) {
        ...
        $(`${section} .faq-toggle-head__item`)
          .not($(this))
          .removeClass('_active-page');
        $(`${section} .faq-toggle-content__item`)
          .not($(goto))
          .removeClass('_active-page');
  ...
  function activateToggleProject(section) {
    $(`${section} .project-toggle-content__item`)
      .not($(`${section} .project-toggle-content__item._active-page`))
      .fadeOut(200);
    ...
    $(`${section} .project-toggle-head__item`).click(function (event) {
      ...
      if (goto && $(goto)) {
        ...
        $(`${section} .project-toggle-head__item`)
          .not($(this))
          .removeClass('_active-page');
        $(`${section} .project-toggle-content__item`)
          .not($(goto))
  ...
  $('.accordion-item .accordion-item__text .accordion-item__subtitle').slideUp(
    0
  );
  ...
  $('.accordion-item').click(function (e) {
    ...
    $(this)
      .children('.accordion-item__text')
      .children('.accordion-item__subtitle')
      .slideToggle(300);
    $(this).toggleClass('_toggle-accordion');

    $('.accordion-item .accordion-item__text .accordion-item__subtitle')
      .not(
        $(this)
          .children('.accordion-item__text')
          .children('.accordion-item__subtitle')
  ...
  if (document.body.clientWidth <= 1250) {
    ...
    $('.transfer-item')
      .children('.transfer-item__title__container._additional')
      .slideUp(0);
    $('.transfer-item')
      .children('.transfer-list._additional .transfer-list__item')
      .slideUp(0);
  }
  ...
  $('.transfer-list__more').click(function () {
    ...
    $(this)
      ...
      .children('.transfer-list__item')
      .not($('.transfer-list__item._first'))
      .slideToggle(300);
  });
  ...
  $('.transfer-item__btn').click(function (e) {
    ...
    const transfer = Array.from(
      document.querySelectorAll('.transfer-item')
    ).filter((item) => item.dataset.item === select);
    ...
    navigator.clipboard
      .writeText(items.map((item) => item.dataset.value).join(',\n '))
      .then(() => (message = 'Copied!'))
  ...
  let dopHeightPojects =
    $('.swiper.swiper-projects .swiper-wrapper').outerHeight(true) -
    $('.swiper.swiper-projects .swiper-wrapper').outerHeight();
  ...
  let dopHeightTeam =
    $('.swiper.swiper-team .swiper-wrapper').outerHeight(true) -
    $('.swiper.swiper-team .swiper-wrapper').outerHeight();
  document.documentElement.style.setProperty(
    '--dop-height-team',
    dopHeightTeam + 'px'
  );
  ...
  function setCurrentCounter(swiper) {
    let currentCounts = document.querySelectorAll(
      '.banner-news__counter .current'
    );
